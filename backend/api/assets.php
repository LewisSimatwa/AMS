...
function handleAssets($db, $method, $path) {
    ...
    elseif ($method === 'POST' && $path === '/assets') {
        try {
            ...
            // Note: asset_code and serial_number are now optional
            // They will be auto-generated by database triggers if not provided
            
            // If asset_code is provided, check if it already exists
            ...
            // Insert asset
            // If asset_code or serial_number are not provided (NULL or empty),
            // the database triggers will auto-generate them
            $stmt = $db->prepare("
                INSERT INTO assets (
                    institution_id,
                    asset_type_id,
                ...
                RETURNING id, asset_code, serial_number
            ");
            
            $stmt->execute([
                $institutionId,
                $assetTypeId,
                !empty($input['asset_code']) ? $input['asset_code'] : null,
                !empty($input['serial_number']) ? $input['serial_number'] : null,
                $input['name'],
            ...
            $assetId = $result['id'];
            $generatedAssetCode = $result['asset_code'];
            $generatedSerialNumber = $result['serial_number'];
            
            error_log("Asset created with ID: $assetId, Asset Code: $generatedAssetCode, Serial Number: $generatedSerialNumber");
            ...
            try {
                ...
                $auditStmt->execute([
                    ...
                    json_encode([
                        ...
                        'auto_generated' => [
                            'asset_code' => empty($input['asset_code']),
                            'serial_number' => empty($input['serial_number'])
                        ]
                    ])
                ]);
            } catch (Exception $e) {
    ...
    elseif ($method === 'PUT' && preg_match('#^/assets/(\d+)$#', $path, $matches)) {
        ...
        try {
            ...
            // Note: asset_code and serial_number can now be updated if needed
            $allowedFields = ['name', 'description', 'serial_number', 'status', 'condition', 'location_id', 'asset_code'];
            
            foreach ($allowedFields as $field) {
                if (isset($input[$field])) {
                    // Special validation for asset_code if being changed
                    if ($field === 'asset_code' && $input[$field] !== $oldAsset['asset_code']) {
                        $checkStmt = $db->prepare("
                            SELECT id FROM assets 
                            WHERE asset_code = ? AND institution_id = ? AND id != ?
                        ");
                        $checkStmt->execute([$input[$field], $institutionId, $assetId]);
                        if ($checkStmt->fetch()) {
                            $db->rollBack();
                            respond(['error' => 'Asset code already exists'], 400);
                        }
                    ...
                    // Special validation for serial_number if being changed
                    if ($field === 'serial_number' && $input[$field] !== $oldAsset['serial_number']) {
                        $checkStmt = $db->prepare("
                            SELECT id FROM assets 
                            WHERE serial_number = ? AND institution_id = ? AND id != ?
                        ");
                        $checkStmt->execute([$input[$field], $institutionId, $assetId]);
                        if ($checkStmt->fetch()) {
                            $db->rollBack();
                            respond(['error' => 'Serial number already exists'], 400);
                        }
                    }
